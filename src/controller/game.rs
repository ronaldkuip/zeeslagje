//! Controller: the #[wasm_bindgen] `Game` struct and its impl block -
//! the only thing JS calls. Orchestrates Model (GameState/AiPlayer)
//! mutation and queries; delegates presentation formatting to `view`.
//! Moved out of lib.rs verbatim (Stage 7 of the refactor plan) as a
//! pure relocation - the individual method-level extractions (View
//! logic currently embedded in some of these methods) follow as
//! separate, smaller, individually-reviewed changes on top of this.

use wasm_bindgen::prelude::*;
use serde::Serialize;

use crate::model::ai::AiPlayer;
use crate::model::fleet::{cell_to_str, generate_board, BoardLayout, Cell, GameState, ResolutionStatus, SalvoResult};

#[wasm_bindgen]
pub struct Game {
    // pub(crate), not bare-private, purely so src/tests.rs (a sibling
    // module of controller::game now, not a descendant of it) keeps the
    // same direct field access it always had — a visibility widening,
    // not a behavior change. Same treatment GameState's fields got in
    // Stage 1 for the identical reason.
    pub(crate) state: GameState,
    pub(crate) ai: AiPlayer,
}
